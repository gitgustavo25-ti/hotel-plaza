...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && basename($_SERVER['PHP_SELF']) === 'users.php') {
    ...
    if (isset($_POST['add'])) {
        if (!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['role']) && !empty($_POST['password'])) {
            if ($_POST['password'] == $_POST['password2']) {
                ...
                $sql = "INSERT INTO users (full_name, email, telefone, documento, password, join_date, permissions, last_login, endereco, observacao) VALUES ('$name', '$email', '$telefone','$documento','$password', '$joinDate', '$role', NOW(), '$endereco', '$observacao')";
                $insert = $db->query($sql);
                if ($insert) {
        ...
        $sql = "UPDATE users SET full_name='$name', email='$email', telefone='$telefone', documento='$documento', permissions='$role' , endereco='$endereco', observacao='$observacao' WHERE id='$toEditID'";
        $update = $db->query($sql);
        if ($update) {
