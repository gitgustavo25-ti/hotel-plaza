      ...
      <?php while ($rows = mysqli_fetch_assoc($result)) :
        ...
        ?>
         <tr>
            ...
            <td><?= date('d/m/Y', strtotime($rows['checkin'])); ?></td>
            <td><?= date('d/m/Y', strtotime($rows['checkout'])); ?></td>
            <td class="telefone"><?= $rows['phone']; ?></td>
            <td><?= $rows['people']; ?></td>
            ...
            <td style="text-align: center; vertical-align: middle;">
                <?php // Mantém os botões se o status for 'pendente' ?>
                <?php if ($rows['aprovacao'] === 'pendente') : ?>
                    <a href="solicitacoes.php?approve=<?= $rows['id']; ?>" class="w3-btn w3-green w3-small" title="Aprovar Solicitação"><span class="glyphicon glyphicon-ok"></span> Aprovar</a>
                    <a href="solicitacoes.php?reject=<?= $rows['id']; ?>" class="w3-btn w3-red w3-small" title="Reprovar Solicitação"><span class="glyphicon glyphicon-remove"></span> Reprovar</a>
                <?php endif; ?>

                <?php // NOVO: Mostra um ícone de 'check' se o status for 'aprovado' ?>
                <?php if ($rows['aprovacao'] === 'aprovado') : ?>
                    <span class="glyphicon glyphicon-ok" style="color: green; font-size: 1.8em;" title="Solicitação Aprovada"></span>
                <?php endif; ?>

                <?php // NOVO: Mostra um ícone de 'X' se o status for 'reprovado' ou 'cancelado' ?>
                <?php if ($rows['aprovacao'] === 'reprovado' || $rows['aprovacao'] === 'cancelado') : ?>
                    <span class="glyphicon glyphicon-remove" style="color: #d9534f; font-size: 1.8em;" title="Solicitação Não Efetivada"></span>
                <?php endif; ?>
            </td>
